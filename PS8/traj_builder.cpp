...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
	...
	// poll the current lateral/angular velocity
	double vel_lin = start_velocity.twist.twist.linear.x;
	double vel_ang = start_velocity.twist.twist.angular.z;
	...
	for (int step_num = 0; step_num < decel_steps; step_num++) {
		...
		if (has_linear_velocity) {
			vel_lin -= accel_max_ * dt_ * vel_lin_dir;
			if (vel_lin * vel_lin_dir < 0.0) {
				vel_lin = 0.0;
			}
		}
		if (has_angular_velocity) {
			vel_ang -= alpha_max_ * dt_ * vel_ang_dir;
			if (vel_ang * vel_ang_dir < 0.0) {
				vel_ang = 0.0;
			}
		}
